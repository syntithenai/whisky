...
SCRAPE_MAX_PAGES_PER_SITE="${SCRAPE_MAX_PAGES_PER_SITE:-30}"
...
dist_db = sqlite3.connect(root_dir / "data/distilleries.db")
dist_db.row_factory = sqlite3.Row
state_db = sqlite3.connect(root_dir / "data/site_crawl_state.db")
state_db.row_factory = sqlite3.Row

distilleries = dist_db.execute(
    "SELECT name, official_site FROM distilleries WHERE official_site LIKE 'http%' ORDER BY name"
).fetchall()

state_rows = state_db.execute(
    "SELECT root_url, last_status FROM sites WHERE site_type='distillery'"
).fetchall()
state_by_url = {normalize_url(row["root_url"]): row["last_status"] for row in state_rows}

queue = []
for row in distilleries:
    name = str(row["name"])
    url = normalize_url(row["official_site"])
    if not url:
        continue
...
print(f"Distillery sync: {not no_distillery_sync}")
...
if dry_run:
    for index, (name, _url, status) in enumerate(queue, 1):
...
def read_latest_status(url: str) -> str | None:
    row = state_db.execute(
        "SELECT last_status FROM sites WHERE site_type='distillery' AND RTRIM(root_url, '/') = ? ORDER BY id DESC LIMIT 1",
        (normalize_url(url),),
    ).fetchone()
...
entries = [
    ...
    for name, url, prior_status in queue
]
...
for row in distilleries:
    name = str(row["name"])
    url = normalize_url(row["official_site"] or "")
    if not url:
        continue
    if site_name_filter and site_name_filter not in name.lower():
        continue
...
report_lines = [
    "# Distillery Scrape Post-Run Report",
    "",
    ...
    f"Distillery sync: {not no_distillery_sync}",
    f"Quiet crawl: {quiet_crawl}",
]

if failures:
    report_lines.extend(["", "## Failed Sites", ""])
    report_lines.extend(f"- {name}" for name in sorted(failures))

if timeouts:
    report_lines.extend(["", "## Timed Out Sites", ""])
    report_lines.extend(f"- {name}" for name in sorted(timeouts))
